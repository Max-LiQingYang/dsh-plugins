...
```
● [进行中] 修复 flaky 测试并在 goal 报告中保留证据…  轮次 3/8  [███████░░░]  耗时 0:12:34  ▸
```

- **状态圆点 + 徽章**：进行中=绿色呼吸闪烁 / 已暂停=琥珀 / 受阻=红色闪烁 / 已完成=绿色
- **轮次计数器**：`roundsStarted / maxGoalRounds`
- **进度条**：实时百分比填充，带过渡动画
- **实时耗时**：每秒跳动（client `timer` 服务驱动）
- **点击展开详情**：完整目标、创建/更新时间、受阻原因（code + message）、完成态总耗时
- 无目标（或目标被 clear）时自动隐藏；`complete` 状态显示绿色完成总结
  （内置 GoalBar 在 complete 时反而不渲染）

## 工作原理

- 数据源：宿主计算的 **`goal` 会话投影**。每次目标变更（`goal/change` 事件）都携带
  完整后置状态，投影折叠为 last-wins 整值——所以 UI 无需轮询或 RPC。
- 渲染：注册进 `conversation.input.dock` slot（list 类型，`id: "goal-tracker"`，
  `order: 20`），通过 slot 标准 props 里的 `useProjection('goal')` 钩子订阅投影。
- 与内置 GoalBar（`id: "goal"`, `order: 10`）互不冲突，职责互补：内置版负责
  pause/resume/edit/clear 操作，本插件负责可视化。
